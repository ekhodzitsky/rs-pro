use smart_house::domain::devices::device::Device;
use smart_house::domain::devices::socket::Socket;
use smart_house::domain::devices::thermometer::Thermometer;
use smart_house::domain::houses::house::House;
use smart_house::domain::rooms::room::Room;
...
fn main() {
    ...
    let report = my_house.create_report(report_devices).unwrap();
    println!("{}", report);

    let empty_device_vec: Vec<Box<dyn Device>> = vec![];
    let err = my_house.create_report(empty_device_vec).unwrap_err();
    println!("{}", err);
}
